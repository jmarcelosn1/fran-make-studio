...
Contagem de visitas: aprovada pelo cliente em 2026-09-24, pela Cloudflare Web Analytics, que não usa cookies nem guarda IP e registra página, origem da visita, navegador, país e tempos de carregamento. Só funciona depois que o responsável ligar a Web Analytics no painel da Cloudflare Pages (Metrics > Web Analytics); a Cloudflare então injeta o script sozinha. A CSP libera apenas `https://static.cloudflareinsights.com` em `script-src` e `https://cloudflareinsights.com` em `connect-src`, e o teste de cabeçalhos confere exatamente esses dois. Não há replay, formulário nem erro de JavaScript enviado a terceiros. Fora isso, há relatórios de teste e erros de navegador coletados na CI. Sentry é a opção a avaliar para erros reais de produção depois de definir projeto/DSN, responsável, retenção e tratamento de dados. Datadog, New Relic e OpenTelemetry simultâneos seriam redundantes para este frontend. Não foram adicionados SDKs sem destino ou contas configuradas.
